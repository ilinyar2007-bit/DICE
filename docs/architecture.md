...
**Состояние:** Игровое состояние хранится полностью в Lua-глобалах (например, таблица `game` в dice_demo). Встроенного механизма undo/redo или снепшотов нет — откат состояния реализуется в скрипте. `Action`/`ActionManager` в разработке.
...
| `Action` / `ActionManager` | Дискретные игровые действия *(в разработке)* | `dice::core` |
...
**Ключевые детали:**
- Скрипты из секции `"scripts"` загружаются **до** `Model::fromJson` — они регистрируют триггеры до создания объектов.
...
- `clearSceneState()` сбрасывает **все** подписки текущей сцены: `scriptRegistry_`, `inlineCallbacks_`, `triggerCatalog_` и `keyHandlers_`. Обработчики `engine.onKey`, зарегистрированные в предыдущей сцене, **не сохраняются** при переходе на новую.
...
| `log` | `(msg: string)` | O(1) | Лог через spdlog |
