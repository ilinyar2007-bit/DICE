...
| `NetworkManager` | Сетевой фасад: Lua-биндинги, делегирование к `HostServer` / `GameClient` | `dice::network` |
...
## 8. Сетевая часть *(демо, нестабильно)*

> ⚠️ Сетевая подсистема находится в демонстрационном состоянии. Возможны зависания соединения, рассинхронизация состояния и неожиданные отключения. Не рекомендуется для продакшн-игр.

DICE поддерживает многопользовательский режим по TCP. Основные классы находятся в `dice::network`.
...
Регистрируются в `NetworkManager::registerLuaBindings()` и доступны глобально из любого Lua-скрипта.
