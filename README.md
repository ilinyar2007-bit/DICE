...
1. Склонируйте репозиторий
   ...
   ```
2. Перейдите в каталог DICE и соберите проект
   ```sh
   cmake . -B build
   cd build
   make
   ```
   CMake автоматически копирует `game.json`, `scenes/`, `scripts/` и `assets/` в папку с бинарником.
3. Запустите приложение из папки `build/`
   ```sh
   ./dice
   ```
